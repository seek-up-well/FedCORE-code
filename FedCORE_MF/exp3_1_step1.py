...
if __name__ == "__main__":
    ...
    model_b_to_a = MF(maxn, maxm, HIDDEN, DROP)
    model_b_to_a.load_state_dict(torch.load(SaveUV_dir_tempb))

    model_b_to_a.state_dict()['iembedding_layer.weight'].copy_(
        model_a.state_dict()['iembedding_layer.weight'])
    model_b_to_a.to(DEVICE)
    model_b_to_a.eval()

    def quandata(data2useruni_list, model_b_to_a, data1itemuni_list, scale_temp):
        ...
        ainfer_user_input = torch.LongTensor(ainfer_user_temp).to(DEVICE)
        ainfer_item_input = torch.LongTensor(ainfer_item_temp).to(DEVICE)

        loc = 0.0
        model_b_to_a.state_dict()['uembedding_layer.weight'].copy_(model_b.state_dict()['uembedding_layer.weight'] + torch.Tensor(
            np.random.laplace(loc, scale_temp, size=model_b.state_dict()['uembedding_layer.weight'].size())).to(DEVICE))
    ...
    ainfer_data = quandata(data2useruni_list, model_b_to_a,
                           data1itemuni_list, scale_temp)
    ...
    model_a_temp = MF(maxn, maxm, HIDDEN, DROP)
    model_a_temp.load_state_dict(torch.load(SaveUV_dir_tempa))
    model_a_temp.to(DEVICE)
    model_a_temp.eval()
    loc = 0.0
    AR_result = []

    for ud in data2useruni_set:

        A = []
        b = []
        temp_num = 0
        for vd in data1itemuni_set:
            if (ainfer_data_dict[(ud, vd)] != 1.0) and (ainfer_data_dict[(ud, vd)] != 5.0):

                b.append(ainfer_data_dict[(ud, vd)])

                A.append(model_a_temp.state_dict()[
                         'iembedding_layer.weight'][vd].cpu().numpy().tolist())
                temp_num = temp_num + 1
                if temp_num == HIDDEN:
                    break
        if len(b) < 20:
        ...
        attack_uembedding = linalg.solve(A, b)
        model_a_temp.state_dict()['uembedding_layer.weight'][ud].copy_(
            torch.Tensor(attack_uembedding))

    test(model_b_to_a, atest_data_non)
    mae, rmse = test(model_a_temp, atest_data_non)
    AR_result.append(mae)
    AR_result.append(rmse)

    AR_result = list(np.around(np.array(AR_result), 4))
    file_temp = open(
        SaveUV_dir + f"scale_{scale_t}_embnoiseARattack.txt", 'w+')
    file_temp.write(f"{AR_result}" + "\n")
    file_temp.close()
    print()
