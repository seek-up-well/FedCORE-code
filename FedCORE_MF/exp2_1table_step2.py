...
def cal__max_sim_num(iidfind, model_a, model_b, data2itemuni_set, top_num):
    ...
    for i in range(len(iidfind)):
        target = torch.cosine_similarity(model_a.state_dict()['iembedding_layer.weight'][iidfind[i][1]], model_b.state_dict()[
                                         'iembedding_layer.weight'][iidfind[i][2]], dim=0)
        temp_sim_list = []
        for item_id in data2itemuni_set:
            if iidfind[i][2] != item_id:
                temp_sim_list.append(torch.cosine_similarity(model_a.state_dict()[
                                     'iembedding_layer.weight'][iidfind[i][1]], model_b.state_dict()['iembedding_layer.weight'][item_id], dim=0))
...
if __name__ == "__main__":
    ...
    np.save(save_five_name+'table_result', np.array(result_temp))
