...
class NeuMF_attack(nn.Module):
    def __init__(self, num_users, num_items, hidden_dim, lamb=0.01, dropout=0, init=None):
        super(NeuMF_attack, self).__init__()
    ...
    def forward(self, users, gmf_item_latent, ncf_item_latent):

        ncf_u_latent = self.dropout(self.ncf_user_embedding(users))
        ncf_i_latent = ncf_item_latent
