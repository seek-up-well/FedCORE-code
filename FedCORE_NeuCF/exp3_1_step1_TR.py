...
SaveUV_dir = f"./exp3/{dataset}-copy{copy}-lr{LR}"
...
def train(model, optim, scheduler, input_data, test_data_uni, test_data_com, test_data_non):
    ...
    for ep in range(EPOCH):
        ...
        for i in tqdm((traingen)):
            ...
            userid_input = torch.LongTensor(userid_input).squeeze().to(DEVICE)
            itemid_input = torch.LongTensor(itemid_input).squeeze().to(DEVICE)
            ratings = torch.Tensor(i[1]).squeeze().to(DEVICE)
        ...
        print('ep = '+str(ep) +
              'aaa=============================================================aaa')
        MAE, RMSE = test(model, test_data_uni)
        test(model, test_data_com)
        test(model, test_data_non)
...
def train_together(model_a, optim_a, scheduler_a, model_b, optim_b, scheduler_b, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non, indexvectorlist, maxn, maxm):
    ...
    torch.save(copy.deepcopy(model_a.state_dict()),
               SaveUV_dir + f"-partner{'a'}")
    torch.save(copy.deepcopy(model_b.state_dict()),
               SaveUV_dir + f"-partner{'b'}")
...
def item_id_tran(maxn, maxm, data1useruni_list, data2useruni_list, datausercom_list, data1itemuni_list, data2itemuni_list, dataitemcom_list, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non):
    ...
    idxlen = round(len(dataitemcom_set_temp))
...
if __name__ == "__main__":
    ...
    SaveUV_dir_tempa = SaveUV_dir + f'-partnera'
    model_a = NeuMF(maxn, maxm, HIDDEN, DROP)
    model_a.load_state_dict(torch.load(SaveUV_dir_tempa))
    model_a.to(DEVICE)
    model_a.eval()

    SaveUV_dir_tempb = SaveUV_dir + f'-partnerb'
    model_b = NeuMF(maxn, maxm, HIDDEN, DROP)
    model_b.load_state_dict(torch.load(SaveUV_dir_tempb))
    model_b.to(DEVICE)
    model_b.eval()

    model_b_to_a = NeuMF(maxn, maxm, HIDDEN, DROP)
    model_b_to_a.load_state_dict(torch.load(SaveUV_dir_tempb))

    model_b_to_a.state_dict()['gmf_item_embedding.weight'].copy_(
        model_a.state_dict()['gmf_item_embedding.weight'])
    model_b_to_a.state_dict()['ncf_item_embedding.weight'].copy_(
        model_a.state_dict()['ncf_item_embedding.weight'])

    model_b_to_a.to(DEVICE)
    model_b_to_a.eval()

    def quandata_with_noise(data2useruni_list, model_b_to_a, data1itemuni_list, scale_temp):

        ainfer_user = []
        ainfer_item = []

        data2useruni_temp = np.array(data2useruni_list, 'int32')
        data1itemuni_temp = np.array(data1itemuni_list, 'int32')
        combinations = np.array(
            list(it.product(data2useruni_temp, data1itemuni_temp)), 'int32')

        ainfer_user = combinations[:, 0]
        ainfer_item = combinations[:, 1]

        ainfer_user_temp = np.expand_dims(ainfer_user, axis=1)
        ainfer_item_temp = np.expand_dims(ainfer_item, axis=1)

        ainfer_user_input = torch.LongTensor(
            ainfer_user_temp).squeeze().to(DEVICE)
        ainfer_item_input = torch.LongTensor(
            ainfer_item_temp).squeeze().to(DEVICE)

        loc = 0.0
        model_b_to_a.state_dict()['gmf_user_embedding.weight'].copy_(model_b.state_dict()['gmf_user_embedding.weight'] + torch.Tensor(
            np.random.laplace(loc, scale_temp, size=model_b.state_dict()['gmf_user_embedding.weight'].size())).to(DEVICE))
        model_b_to_a.state_dict()['ncf_user_embedding.weight'].copy_(model_b.state_dict()['ncf_user_embedding.weight'] + torch.Tensor(
            np.random.laplace(loc, scale_temp, size=model_b.state_dict()['ncf_user_embedding.weight'].size())).to(DEVICE))

        preds = model_b_to_a(ainfer_user_input, ainfer_item_input).squeeze()
        preds = torch.where(preds > 5, torch.full_like(preds, 5), preds)
        preds = torch.where(preds < 1, torch.full_like(preds, 1), preds)
        preds = preds.to('cpu').detach().numpy()

        ainfer_data = [ainfer_user, ainfer_item, preds]

        return ainfer_data

    def quandata(data2useruni_list, model_b_to_a, data1itemuni_list):
    ...
    scale_temp = float(scale_t)
    ainfer_data = quandata_with_noise(
        data2useruni_list, model_b_to_a, data1itemuni_list, scale_temp)

    [ainfer_user, ainfer_item, ainfer_label] = ainfer_data

    ainfer_data_dict = {}
    for i in range(len(ainfer_data[0])):
        ainfer_data_dict[(ainfer_data[0][i], ainfer_data[1][i])
                         ] = ainfer_data[2][i]

    scale = 0.0
    loc = 0.0
    ainfer_label_temp = ainfer_label + \
        np.random.laplace(loc, scale, size=ainfer_label.shape)
    permutation = np.random.permutation(ainfer_label_temp.shape[0])

    ainfer_label_temp = ainfer_label_temp[permutation]
    ainfer_user_temp = ainfer_user[permutation]
    ainfer_item_temp = ainfer_item[permutation]

    ratio = float(ratio_t)

    train_ainfer_user_temp = ainfer_user_temp[0:math.floor(
        ratio * len(ainfer_user_temp))]
    train_ainfer_item_temp = ainfer_item_temp[0:math.floor(
        ratio * len(ainfer_item_temp))]
    train_ainfer_label_temp = ainfer_label_temp[0:math.floor(
        ratio * len(ainfer_label_temp))]

    atrain_data_temp = [0, train_ainfer_user_temp,
                        train_ainfer_item_temp, train_ainfer_label_temp]

    model = NeuMF(maxn, maxm, hidden_dim=HIDDEN, dropout=DROP)
    model.to(DEVICE)

    for name, param in model_a.state_dict().items():
        model.state_dict()[name].copy_(model_a.state_dict()[name])
    for k, v in model.named_parameters():
        if (k != 'gmf_user_embedding.weight') and (k != 'ncf_user_embedding.weight'):
    ...
    scheduler = scheduler = torch.optim.lr_scheduler.ExponentialLR(
        optimizer=optimizer, gamma=0.92)
    if not ('cpu' in DEVICE):
        model = model.cuda()
    logger.info("Start training")

    MAE, RMSE = train(model, optimizer, scheduler, atrain_data_temp,
                      atest_data_non, atrain_data_temp[1:], atest_data_non)

    sim_sum = 0
    for i in range(len(data2useruni_list)):

        sim_sum = sim_sum + torch.cosine_similarity(model.state_dict()['ncf_user_embedding.weight'][data2useruni_list[i]], model_b.state_dict()[
                                                    'ncf_user_embedding.weight'][data2useruni_list[i]], dim=0)
    print(sim_sum/len(data2useruni_list))
    print()

    logger.info(f"MAE{MAE}-RMSE{RMSE}")
    TR_result = []
    TR_result.append(MAE)
    TR_result.append(RMSE)
    TR_result = list(np.around(np.array(TR_result), 4))
    file_temp = open(
        SaveUV_dir + f"newscale{scale_t}_ratio{ratio}_TR_result.txt", 'w+')
    file_temp.write(f"{TR_result}" + "\n")
    file_temp.close()
    print()
