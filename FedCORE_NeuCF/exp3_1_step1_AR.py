...
if __name__ == "__main__":
    ...
    test_data_pred_list = []
    for i in range(len(atest_data_non[0])):
        test_data_label_list.append(atest_data_non[2][i])
        test_data_pred_list.append(
            ainfer_data_dict[(atest_data_non[0][i], atest_data_non[1][i])])

    mae, rmse = test_acc(test_data_label_list, test_data_pred_list)
    print(mae, rmse)
    infer_acc_result.append(mae)
    infer_acc_result.append(rmse)

    infer_acc_result = list(np.around(np.array(infer_acc_result), 4))
    file_temp = open(
        SaveUV_dir + f"embnoise_scale{float(scale_t)}infer_acc.txt", 'w+')
    file_temp.write(f"{infer_acc_result}" + "\n")
    file_temp.close()
    print()
