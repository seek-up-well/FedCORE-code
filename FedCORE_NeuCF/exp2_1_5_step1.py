...
exp_dir_temp = f"exp2-1-5/{dataset}-testtpye{testtype}-eps{args.eps}-clip{args.clipnum}-copy{copy}-lr{LR}"
...
SaveUV_dir = f"exp2-1-5/{dataset}-copy{copy}-lr{LR}"
...
def index_to_sum_vector(indexlist, large_num):
    resul = np.zeros(large_num+3, dtype='int32')
    for i in range(len(indexlist)):
        resul[indexlist[i]] = resul[indexlist[i]] + 1
    return resul


def train_together(model_a, optim_a, scheduler_a, model_b, optim_b, scheduler_b, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non, id_vector, maxn, maxm, id_set, id_change, clip_num):
    global LR
    ...
    for ep in range(EPOCH):
        ...
        for i in tqdm((traingen)):
            ...
            userid_index_sum_vector_a = index_to_sum_vector(
                userid_input_a, maxn)
            itemid_index_sum_vector_a = index_to_sum_vector(
                itemid_input_a, maxm)
            userid_index_sum_vector_a = torch.LongTensor(
                userid_index_sum_vector_a).to(DEVICE)
            itemid_index_sum_vector_a = torch.LongTensor(
                itemid_index_sum_vector_a).to(DEVICE)
            ...
            userid_index_sum_vector_b = index_to_sum_vector(
                userid_input_b, maxn)
            itemid_index_sum_vector_b = index_to_sum_vector(
                itemid_input_b, maxm)
            userid_index_sum_vector_b = torch.LongTensor(
                userid_index_sum_vector_b).to(DEVICE)
            itemid_index_sum_vector_b = torch.LongTensor(
                itemid_index_sum_vector_b).to(DEVICE)
            ...
            for j in cur_model_weights_a.keys():
                ...
                if j == 'uembedding_layer.weight':
                    ...
                    logger.debug(
                        f"itemid_b_vector size: {itemid_b_vector.size()}")
                    logger.debug(
                        f"itemid_b_vector input sum: {itemid_b_vector.sum()}")

                    logger.debug(
                        f"data2itemuni * itemid_b_vector size: {(data2itemuni * itemid_b_vector).size()}")
                    logger.debug(
                        f"data2itemuni * itemid_b_vector input sum: {(data2itemuni * itemid_b_vector).sum()}")
...
def train_together_scale(model_a, optim_a, scheduler_a, model_b, optim_b, scheduler_b, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non, id_vector, maxn, maxm, id_set, id_change, scale_temp, clip_num):
    global LR
    print(LR)
    train_user_index_a, trainu_a, traini_a, trainlabel_a = atrain_data
    ...
    for ep in range(EPOCH):
        ...
        for i in tqdm((traingen)):
            ...
            itemid_index_sum_vector_a = index_to_sum_vector(
                itemid_input_a, maxm)
            userid_index_sum_vector_a = torch.LongTensor(
                userid_index_sum_vector_a).squeeze().to(DEVICE)
            itemid_index_sum_vector_a = torch.LongTensor(
                itemid_index_sum_vector_a).squeeze().to(DEVICE)

            userid_a_vector = indexlistto01vector(userid_input_a, maxn)
            itemid_a_vector = indexlistto01vector(itemid_input_a, maxm)
            userid_a_vector = torch.LongTensor(
                userid_a_vector).squeeze().to(DEVICE)
            itemid_a_vector = torch.LongTensor(
                itemid_a_vector).squeeze().to(DEVICE)

            userid_input_a = torch.LongTensor(
                userid_input_a).squeeze().to(DEVICE)
            itemid_input_a = torch.LongTensor(
                itemid_input_a).squeeze().to(DEVICE)
            ratings_a = torch.Tensor(i[1]).squeeze().to(DEVICE)
            ...
            itemid_index_sum_vector_b = index_to_sum_vector(
                itemid_input_b, maxm)
            userid_index_sum_vector_b = torch.LongTensor(
                userid_index_sum_vector_b).squeeze().to(DEVICE)
            itemid_index_sum_vector_b = torch.LongTensor(
                itemid_index_sum_vector_b).squeeze().to(DEVICE)

            userid_b_vector = indexlistto01vector(userid_input_b, maxn)
            itemid_b_vector = indexlistto01vector(itemid_input_b, maxm)
            userid_b_vector = torch.LongTensor(
                userid_b_vector).squeeze().to(DEVICE)
            itemid_b_vector = torch.LongTensor(
                itemid_b_vector).squeeze().to(DEVICE)

            userid_input_b = torch.LongTensor(
                userid_input_b).squeeze().to(DEVICE)
            itemid_input_b = torch.LongTensor(
                itemid_input_b).squeeze().to(DEVICE)
            ratings_b = torch.Tensor(i[3]).squeeze().to(DEVICE)
            ...
            for j in cur_model_weights_a.keys():
                ...
                model_a_grads = pre_model_weights_a[j]-cur_model_weights_a[j]
                model_b_grads = pre_model_weights_b[j]-cur_model_weights_b[j]
                if (j == 'gmf_user_embedding.weight') or (j == 'ncf_user_embedding.weight'):
                    ...
                    loc_list = torch.zeros(
                        size=[BATCH_SIZE*2, HIDDEN], dtype=torch.float)
                    scale_list = torch.full([BATCH_SIZE*2, HIDDEN], scale_temp)
                    la = torch.distributions.laplace.Laplace(
                        loc=loc_list, scale=scale_list)
                    la_sample = la.sample()

                    index1 = (
                        (userid_a_vector*userid_b_vector).unsqueeze(1).expand(-1, HIDDEN))

                    non_zeor_index1 = torch.nonzero(
                        index1[:, 0] == 1).squeeze(1)

                    la_sample_size1 = non_zeor_index1.size()[0]

                    avg_grads1 = index1*model_a_grads
                    avg_grads1[non_zeor_index1] = clip_by_l1norm(
                        avg_grads1[non_zeor_index1]/LR)
                    avg_grads1[non_zeor_index1] = avg_grads1[non_zeor_index1] * \
                        LR + la_sample[0:la_sample_size1].to(DEVICE)*LR

                    avg_grads2 = index1*model_b_grads
                    avg_grads2[non_zeor_index1] = clip_by_l1norm(
                        avg_grads2[non_zeor_index1]/LR)
                    avg_grads2[non_zeor_index1] = avg_grads2[non_zeor_index1] * LR + \
                        la_sample[la_sample_size1:la_sample_size1 +
                                  la_sample_size1].to(DEVICE)*LR

                    avg_grads = (
                        avg_grads1 + avg_grads2
                    )/2
                    ...
                    non_zeor_index2 = torch.nonzero(
                        a_temp[:, 0] == 1).squeeze(1)

                    la_sample_size2 = non_zeor_index2.size()[0]

                    avg_grads_temp1 = (a_temp*model_a_grads)
                    avg_grads_temp1[non_zeor_index2] = clip_by_l1norm(
                        avg_grads_temp1[non_zeor_index2]/LR)
                    avg_grads_temp1[non_zeor_index2] = avg_grads_temp1[non_zeor_index2] * LR + \
                        la_sample[la_sample_size1*2:la_sample_size1 *
                                  2+la_sample_size2].to(DEVICE)*LR
                    ...
                    non_zeor_index3 = torch.nonzero(
                        b_temp[:, 0] == 1).squeeze(1)

                    la_sample_size3 = non_zeor_index3.size()[0]

                    avg_grads_temp2 = (b_temp*model_b_grads)
                    avg_grads_temp2[non_zeor_index3] = clip_by_l1norm(
                        avg_grads_temp2[non_zeor_index3]/LR)
                    avg_grads_temp2[non_zeor_index3] = avg_grads_temp2[non_zeor_index3] * LR + la_sample[la_sample_size1 *
                                                                                                         2+la_sample_size2:la_sample_size1*2+la_sample_size2+la_sample_size3].to(DEVICE)*LR
                ...
                elif (j == 'gmf_item_embedding.weight') or (j == 'ncf_item_embedding.weight'):
...
def test(model, input_data):
    ...
    userid_input = torch.LongTensor(testu).squeeze().to(DEVICE)
    itemid_input = torch.LongTensor(testi).squeeze().to(DEVICE)
    ratings = torch.Tensor(testlabel).squeeze().to(DEVICE)
...
def model_init(maxn, maxm, hidden_dim=HIDDEN, dropout=DROP, lr=LR):
    model = NeuMF(maxn, maxm, hidden_dim, dropout)

    optimizer = torch.optim.SGD(model.parameters(), lr)
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=1, gamma=0.90, last_epoch=-1)
...
if __name__ == "__main__":
    ...
    iidfind, data1itemchange, data2itemchange, maxn, maxm, data1useruni_list, data2useruni_list, datausercom_list, data1itemuni_list, data2itemuni_list, dataitemcom_list, data1useruni_set, data2useruni_set, datausercom_set, data1itemuni_set, data2itemuni_set, dataitemcom_set, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non = item_id_tran(
        maxn, maxm, data1useruni_list, data2useruni_list, datausercom_list, data1itemuni_list, data2itemuni_list, dataitemcom_list, atrain_data, atest_data_uni, atest_data_com, atest_data_non, btrain_data, btest_data_uni, btest_data_com, btest_data_non)
    ...
    id_vector = [data1useruni, data2useruni, datausercom,
                 data1itemuni, data2itemuni, dataitemcom]
    id_set = [data1useruni_set, data2useruni_set, datausercom_set,
              data1itemuni_set, data2itemuni_set, dataitemcom_set]
    id_change = [iidfind, data1itemchange, data2itemchange]
    ...
    np.save(SaveUV_dir+'iidfind', iidfind)
    ...
    if args.eps == "0":
        train_together(model_a, optimizer_a, scheduler_a, model_b, optimizer_b, scheduler_b, atrain_data, atest_data_sum, atest_data_sum,
                       atest_data_non, btrain_data, btest_data_sum, btest_data_sum, btest_data_non, id_vector, maxn, maxm, id_set, id_change, clip_num)
    else:
        train_together_scale(model_a, optimizer_a, scheduler_a, model_b, optimizer_b, scheduler_b, atrain_data, atest_data_sum, atest_data_sum,
                             atest_data_non, btrain_data, btest_data_sum, btest_data_sum, btest_data_non, id_vector, maxn, maxm, id_set, id_change, scale_temp, clip_num)
